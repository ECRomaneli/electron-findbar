...
const Remote: Remote = {
  ...
  inputChange: (text) => { ipcRenderer.send('electron-findbar/input-change', text); },
  matchCase: (value) => { ipcRenderer.send('electron-findbar/match-case', value); },
  previous: () => { ipcRenderer.send('electron-findbar/previous'); },
  next: () => { ipcRenderer.send('electron-findbar/next'); },
  open: () => { ipcRenderer.send('electron-findbar/open'); },
  ...
  onMatchCaseChange: (listener) => { ipcRenderer.on('electron-findbar/match-case-change', listener); },
  onForceTheme: (listener) => { ipcRenderer.on('electron-findbar/force-theme', listener); },
};
